...
public class EasySpecification<T> implements Specification<T> {
    ...
    // 缓存join,join对象复用
    private Map<String, Path> cacheMap = new ConcurrentHashMap<>();
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        if (andTerms == null && orTerms == null && joinTerms == null) {
    ...
    private Predicate handleAnd(Root<T> root, CriteriaBuilder cb) {
       Predicate predicate = null;
        for (Term term : andTerms) {
            predicate = predicate == null
                    ? cb.and(just(root, cb, term)) : cb.and(just(root, cb, term), predicate);
        }
        return predicate;
    }
    ...
    private Predicate handleOr(Root<T> root, CriteriaBuilder cb) {
        ...
        for (Term term : orTerms) {
            predicate = predicate == null ? cb.or(just(root, cb, term)) : cb.or(just(root, cb, term), predicate);
        }
        return predicate;
    }
    ...
    private Predicate handleJoin(Root<T> root, CriteriaBuilder cb) {
        ...
        for (Term joinTerm : joinTerms) {
            ...
            if (predicate == null) {
                ...
                predicate = joinTerm.and ? cb.and(p) : cb.or(p);
            } else {
                predicate = joinTerm.and ? cb.and(predicate, p) : cb.or(predicate, p);
            }
        }
        return predicate;
    }
